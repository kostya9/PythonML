...
for i = 1:m
	...
	y_cur(y(i)) = 1;
	J = J - (1/m) * sum(y_cur .* log(h) + (1 - y_cur) .* log(1 - h));
	%%%BACK PROP%%%
	_Theta2 = Theta2;
	_Theta1 = Theta1;
	_Theta1(:, 1) = [];
	...
	delta2 = (_Theta2' * delta3) .* sigmoidGradient(z2);
	DELTA1 = DELTA1 + delta2*_X(i, :);
	DELTA2 = DELTA2 + delta3*_a2';
endfor		
%%%BACK PROP%%%
...
_Theta1 = _Theta1.^2;
_Theta2 = _Theta2.^2;
J = J + (lambda/(2 * m)) * ( sum(_Theta1(:)) + sum(_Theta2(:)) );
